...
- 名前: `apiwf` (パッケージ) / `toml-http-flow` (リポジトリ)
- 種別: CLI ツール
...
3. **生成スクリプトの自己完結性**: `apiwf/templates/runner.py.tmpl` から生成される .py は
   `apiwf` パッケージを import せずに単体で動作しなければならない。
...
| パス | 責務 | 編集時の注意 |
|------|------|--------------|
| [apiwf/config.py](apiwf/config.py) | TOML → `@dataclass` 変換 / バリデーション | `parse_kv_list` の挙動を変える時は設計書 §4.4 も更新 |
| [apiwf/template.py](apiwf/template.py) | `${...}` 展開 / `$$` エスケープ | 正規表現 `PATTERN` は generator の同等品と一致させる |
| [apiwf/httpclient.py](apiwf/httpclient.py) | `urllib` で HTTP送信 / JSONパス抽出 | `extract()` のロジックは generator にも反映 |
| [apiwf/workflow.py](apiwf/workflow.py) | ステップ実行 / 変数ストア管理 | `store = {"vars": ..., "steps": ...}` の構造は固定 |
| [apiwf/cli.py](apiwf/cli.py) | `argparse` ディスパッチ | `run` 省略時に `run` 扱いとする後方互換を保つ |
| [apiwf/generator.py](apiwf/generator.py) | TOML → 単一 .py 生成 | 出力は必ず `compile()` で構文検証可能なこと |
| [apiwf/templates/runner.py.tmpl](apiwf/templates/runner.py.tmpl) | 生成スクリプトのベース | プレースホルダ `{{REQUESTS}}` `{{DEFAULT_VARS}}` `{{VERSION}}` `{{GENERATED_AT}}` のみ置換 |
...
python3 -m apiwf generate -f <some.toml> -o /tmp/g.py
...
- `apiwf` 本体を import する形での生成スクリプト書き出し
