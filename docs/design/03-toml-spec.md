...
[[requests]]
name    = "uploadRaw"
request = "PUT https://api.example.com/files/${var.file_id}"
...
| request      | ○    | string         | `"METHOD URL"` 形式の1行。HTTPメソッド（GET/POST/PUT/DELETE 等）または特殊メソッド（`SLEEP`）と、URL（`SLEEP` の場合は秒数等のパラメータ）を先頭の空白で区切って指定する |
...
### 3.3.1 `request` フィールドのパース規則

- `request` は必須。`"METHOD URL"` のように、先頭の空白でメソッドとURLの2つに分割する。
- メソッド部は大文字小文字を区別せず、内部では大文字に正規化する。
- URL部は先頭・末尾の空白をトリムする。URL部が空の場合はエラー。
- URLに空白を含めることはできない（URLは空白を百分率エンコードする前提）。
- 旧仕様の `method` / `url` フィールドは廃止された。`request` に統合されている。
  旧形式で書かれた TOML を読み込むと、`request` が無い旨とともに移行を促すエラーになる。
...
`request = "SLEEP <seconds>"` とすることで、指定秒数の待機（`time.sleep`）を行うステップを挿入できる。
...
- `request` のメソッド部に `SLEEP` を指定し、URL部に待機秒数を指定する（テンプレート変数も使用可）。
