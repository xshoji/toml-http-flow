...
加えて、ワークフローTOMLから **本ツールに依存しない単一のスクリプト** を
書き出す機能を備える。既定は bash / curl を使う `.sh`、`--format python` で
Python スクリプト（`.py`）。証跡保存・他者への共有・CI/CD への組み込みなど、
...
| 出力         | 標準出力                                                          |
| スクリプト生成 | `generate` サブコマンドでワークフローを独立した単一スクリプト（既定: bash、`--format python` で .py）に変換  |
