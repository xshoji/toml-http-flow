...
### 3.2 apiwf/template.py

- 正規表現 `r"\$\{([\w\.]+)\}"` で `${...}` 形式の変数参照を検知
...
```bash
# ワークフローを実行（基本）
python -m apiwf run -f workflow.toml
python -m apiwf     -f workflow.toml          # run 省略形

# 変数注入
python -m apiwf run -f workflow.toml -v env=production -v user_id=123

# 単一の Python スクリプトを生成
python -m apiwf generate -f workflow.toml -o workflow.py
python -m apiwf generate -f workflow.toml    # 標準出力に出力
```
...
生成される `workflow.py` は以下のセクションで構成される。

```
#!/usr/bin/env python3          ← --shebang 指定時のみ
"""                             ← ヘッダーコメント
Generated by apiwf vX.Y.Z from workflow.toml at 2026-05-19T...
...
def do_request(method, url, headers, body_bytes): ...
...
DEFAULT_VARS = {"env": "production"}   # -v 引数で上書き可能
...
これらの関数は `apiwf/template.py` `apiwf/httpclient.py` の実装と
...
```bash
# 生成
python -m apiwf generate -f workflow.toml -o audit/workflow_2026-05-19.py
...
| 起動コマンド | `./api-workflow-cli`          | `python -m apiwf` / `apiwf`               |
